...
create table customers (
    cust_id     varchar2(20)
                ...
                constraint customers_ck_cust_id check (lengthb(cust_id) >= 8),
    cust_pw     varchar2(20) not null
                constraint customers_ck_cust_pw check (lengthb(cust_pw) >= 8),
    cust_email  varchar2(100),
    cust_gender number(1) default 0
                constraint customers_ck_cust_gender check (cust_gender in (0, 1, 2)),
    cust_age    number(3) default 0
                constraint customers_ck_cust_age check (cust_age between 0 and 200)
);
 ...
 */
 create table orders (
    ...
    constraint orders_ck_order_method check (order_method in ('online', 'offline')),
